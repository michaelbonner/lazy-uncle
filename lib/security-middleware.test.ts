...
describe("SecurityMiddleware", () => {
  ...
  describe("checkSubmissionSecurity", () => {
    ...
    it("should block and deactivate link for high-risk suspicious activity", async () => {
      ...
      vi.mocked(prisma.birthdaySubmission.count).mockResolvedValue(2);
      vi.mocked(prisma.sharingLink.update).mockResolvedValue({} as any);
  ...
  describe("suspicious content detection", () => {
    it("should detect HTML tags", () => {
      ...
      testCases.forEach((content) => {
        ...
        const result = (SecurityMiddleware as any).containsSuspiciousContent(
          content,
        );
  ...
  describe("bot detection", () => {
    it("should detect common bot user agents", () => {
      ...
      botUserAgents.forEach((userAgent) => {
        const context: SecurityContext & { userId: string } = {
          ipAddress: "192.168.1.1",
          userAgent,
          userId: "user123",
        };
