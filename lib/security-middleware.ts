...
export class SecurityMiddleware {
  ...
  private static async detectSuspiciousLinkGeneration(
    ...
    // Check for rapid link generation from same IP
    const recentLinksFromIp = await prisma.sharingLink.count({
      where: {
        createdAt: {
          gte: new Date(
            Date.now() -
        ...
        // Note: We don't have IP tracking in the schema yet, so this is a placeholder
        // In a real implementation, we'd add an ipAddress field to SharingLink
      },
    });
  ...
  private static async detectSuspiciousSubmission(
    ...
    const recentSubmissionsFromIp = await prisma.birthdaySubmission.count({
      where: {
        createdAt: {
          gte: new Date(
            Date.now() - this.SUSPICIOUS_PATTERNS.RAPID_SUBMISSIONS.windowMs,
          ),
        },
        // Note: We don't have IP tracking in submissions yet
        // This would require adding ipAddress field to BirthdaySubmission schema
      },
    });
   ...
   */
  static async getSecurityStats(timeWindowHours: number = 24): Promise<{
