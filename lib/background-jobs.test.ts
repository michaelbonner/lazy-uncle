...
describe("BackgroundJobScheduler", () => {
  ...
  describe("start", () => {
    ...
    it("should clean up expired links every hour", async () => {
      ...
      (SharingService.cleanupExpiredLinks as any).mockResolvedValue(5);
    ...
    it("should clean up old submissions every 6 hours", async () => {
      ...
      (
        SubmissionService.cleanupOldRejectedSubmissions as any
      ).mockResolvedValue(3);
    ...
    it("should handle notification processing errors gracefully", async () => {
      const { notificationService } = await import("./notification-service");
      (
        notificationService.processPendingNotifications as any
    ...
    it("should handle link cleanup errors gracefully", async () => {
      const { SharingService } = await import("./sharing-service");
      (SharingService.cleanupExpiredLinks as any).mockRejectedValue(
  ...
  describe("daily summary processing", () => {
    ...
    it("should clean up orphaned data every 12 hours", async () => {
      ...
      (prisma.default.birthdaySubmission.deleteMany as any).mockResolvedValue({
        count: 1,
      });
      (
        prisma.default.notificationPreference.deleteMany as any
    ...
    it("should perform database maintenance every 24 hours", async () => {
      const prisma = await import("./prisma");
      (prisma.default.user.count as any).mockResolvedValue(10);
      (prisma.default.birthday.count as any).mockResolvedValue(50);
      (prisma.default.sharingLink.count as any).mockResolvedValue(5);
      (prisma.default.birthdaySubmission.count as any).mockResolvedValue(20);
  ...
  describe("runMaintenanceJob", () => {
    it("should run expired links cleanup job manually", async () => {
      const { SharingService } = await import("./sharing-service");
      (SharingService.cleanupExpiredLinks as any).mockResolvedValue(3);
    ...
    it("should run orphaned data cleanup job manually", async () => {
      const prisma = await import("./prisma");
      // Mock all three deleteMany calls that happen in cleanupOrphanedData
      (prisma.default.birthdaySubmission.deleteMany as any)
        .mockResolvedValueOnce({ count: 1 }) // orphaned submissions
        .mockResolvedValueOnce({ count: 1 }); // old imported submissions
      (
        prisma.default.notificationPreference.deleteMany as any
      ).mockResolvedValue({ count: 0 });
    ...
    it("should run database maintenance job manually", async () => {
      const prisma = await import("./prisma");
      (prisma.default.user.count as any).mockResolvedValue(5);
      (prisma.default.birthday.count as any).mockResolvedValue(25);
      (prisma.default.sharingLink.count as any).mockResolvedValue(3);
      (prisma.default.birthdaySubmission.count as any).mockResolvedValue(10);
